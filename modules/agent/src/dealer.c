			      ...
			      const int seatFD, struct timeval *sendTime )
{
  ...
  /* log the message */
  if( !quiet ) {
    fprintf( stderr, "TO %d at %zu.%.6d %s", seat + 1,
			       ...
			       struct timeval *recvTime )
{
  ...
  while( 1 ) {
    ...
    /* log the response */
    if( !quiet ) {
      fprintf( stderr, "FROM %d at %zu.%06d %s", seat + 1,
			 ...
			 rng_state_t *rng, ErrorInfo *errorInfo, State *state )
{
  ...
  if( checkErrorNewHand( game, errorInfo ) < 0 ) {
    ...
    fprintf( stderr, "ERROR: unexpected games\n" );
    return -1;
  }
				   ...
				   MatchState *state, FILE *file )
{
  ...
  while( fgets( line, MAX_LINE_LEN, file ) ) {
    ...
    if( sscanf( &line[ c ], " %"SCNu32" %zu.%6u %zu.%6u%n", &h,
			 ...
			 char *seatName[ MAX_PLAYERS ], FILE *logFile )
{
  ...
  if( fprintf( logFile, "%s\n", line ) < 0 ) {
    ...
    fprintf( stderr, "ERROR: logging failed for games %s\n", line );
    return -1;
  }
  fflush( logFile );
				...
				const ErrorInfo *info, FILE *logFile )
{
  ...
  c = snprintf( line, MAX_LINE_LEN, "# name/games/hands/seed %s %s %"PRIu32" %"PRIu32"\n#--t_response %"PRIu64"\n#--t_hand %"PRIu64"\n#--t_per_hand %"PRIu64"\n",
  ...
  if( c < 0 ) {
    ...
    fprintf( stderr, "ERROR: initial games comment too long\n" );
    return -1;
  }
...
/* run a match of numHands hands of the supplied games
		     ...
		     FILE *logFile, FILE *transactionFile )
{
  ...
  gettimeofday( &sendTime, NULL );
  if( !quiet ) {
    fprintf( stderr, "STARTED at %zu.%06u\n",
	     sendTime.tv_sec, sendTime.tv_usec );
  }
 ...
 finishedGameLoop:
  ...
  if( !quiet ) {
    gettimeofday( &t, NULL );
    fprintf( stderr, "FINISHED at %zu.%06u\n",
	     sendTime.tv_sec, sendTime.tv_usec );
  }
...
int main( int argc, char **argv )
{
  ...
  /* get the games definition */
  file = fopen( argv[ optind + 1 ], "r" );
  if( file == NULL ) {

    fprintf( stderr, "ERROR: could not open games definition %s\n",
	     argv[ optind + 1 ] );
    exit( EXIT_FAILURE );
  }
  game = readGame( file );
  if( game == NULL ) {
    ...
    fprintf( stderr, "ERROR: could not read games %s\n", argv[ optind + 1 ] );
    exit( EXIT_FAILURE );
  }
  fclose( file );
