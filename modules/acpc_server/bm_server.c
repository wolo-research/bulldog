...
typedef struct {
  ...
  uint16_t maxRunningJobs; /* maximum simultaneous jobs at a time for games
...
/* returns entry for games on success, NULL on failure */
...
void readConfig( const char *filename, Config *conf )
{
  ...
  while( fgets( lineBuf, READBUF_LEN, file ) ) {
	...
	fprintf( stderr, "BM_ERROR: server port must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
    ...
    } else if( strncasecmp( line, "games", 4 ) == 0 ) {
      FILE *file;
      GameConfig gc;
	...
	fprintf( stderr, "BM_ERROR: can't define a games within another games block\n" );
	exit( EXIT_FAILURE );
      }
      if( sscanf( &line[ 4 ], " %s", game ) < 1 ) {

	fprintf( stderr, "BM_ERROR: could not get games name from: %s", line );
	exit( EXIT_FAILURE );
      }
      if( findGame( conf, game ) ) {

	fprintf( stderr, "BM_ERROR: games %s has already been used\n", game );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: could not open games file %s\n", gc.gameFile );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: could not read games %s", gc.gameFile );
	exit( EXIT_FAILURE );
      }
      gameConf
	= (GameConfig *)LLPoolGetItem( LLPoolAddItem( conf->games, &gc ) );
    } else if( strncmp( line, "}", 1 ) == 0 ) {
      /* finished games definition */
	...
	fprintf( stderr, "BM_ERROR: maxRunningBots must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: startupTimeoutSecs must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: responseTimeoutSecs must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: handTimeoutSecs must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: avgHandTimeSecs must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: maxMatchRuns must be defined within a games block\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: maxRunningJobs must be defined within a games block\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr, "BM_ERROR: matchHands must be defined within a games block\n" );
	exit( EXIT_FAILURE );
      }
	...
	fprintf( stderr,
		 "BM_ERROR: users must be defined outside of games blocks\n" );
	exit( EXIT_FAILURE );
      }
      addUser( conf, &line[ 4 ] );
    } else {
...
void writeHelpMessage( int fd )
{
  ...
  r = write( fd, "RUNMATCHES games #runs tag rngSeed player ... - submit match request\n", 68 );
...
int startMatchJob( const Config *conf, ServerState *serv )
{
  ...
  for( cur = LLPoolFirstEntry( serv->matches ); cur != NULL; cur = next ) {
	...
	>= curMatch->gameConf->maxRunningJobs ) {
      /* cur refers to a match in a games which is currently too busy */
